...
const WalletList = ({
  ...
  const saveWalletLabel = async (walletAddress, label) => {
    ...
    if (token) {
      try {
        const response = await fetch('http://localhost:5555/api/wallets/label', {
          method: 'POST',
          headers: {
    ...
    if (token) {
      try {
        const response = await fetch(
          `http://localhost:5555/api/wallets/label?walletAddress=${walletAddress}&cryptoType=${selectedCrypto}`,
          {
            method: 'DELETE',
            headers: {
              Authorization: token,
