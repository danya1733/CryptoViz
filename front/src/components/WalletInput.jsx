...
const WalletInput = ({ walletInput, setWalletInput, addedWallets, setAddedWallets, getTRC20Transfers, getBTCTransfers, getETHTransfers, getERC20Transfers, updateNodeColorsAndImages, selectedCrypto, setSelectedCrypto, userApiKey, setUserApiKey }) => {
  ...
  const addWallet = async () => {
    ...
    if (selectedCrypto === 'usdt') {
      const invalidAddresses = walletAddresses.filter((walletAddress) => !TronWeb.isAddress(walletAddress));
      if (invalidAddresses.length > 0) {
        alert(`Некорректный адрес: ${invalidAddresses.join(', ')}`);
        return;
      }
    }
    ...
    for (const walletAddress of walletAddresses) {
      if (!newAddedWallets.includes(walletAddress)) {
        try {
          ...
          const response = await fetch('http://localhost:5555/api/wallets/add', {
            method: 'POST',
            headers: {
  ...
  const saveApiKey = async (apiKey) => {
    try {
      ...
      if (token) {
        const response = await fetch('http://localhost:5555/api/wallets/apiKey', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': token
