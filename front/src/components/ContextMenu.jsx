...
const ContextMenu = ({
  ...
  const setImage = (nodeId, imageType) => {
    ...
    if (token) {
      fetch('http://localhost:5555/api/wallets/image', {
        method: 'POST',
        headers: {
  ...
  const deleteImage = (nodeId) => {
    ...
    const token = localStorage.getItem('token');
    if (token) {
      fetch(`http://localhost:5555/api/wallets/image?walletAddress=${nodeId}&cryptoType=${selectedCrypto}`, {
        method: 'DELETE',
        headers: {
          'Authorization': token
        }
  ...
  const deleteLabel = (nodeId) => {
    ...
    const token = localStorage.getItem('token');
    if (token) {
      fetch(`http://localhost:5555/api/wallets/label?walletAddress=${nodeId}&cryptoType=${selectedCrypto}`, {
        method: 'DELETE',
        headers: {
          'Authorization': token
        }
  ...
  const updateWalletLabel = (walletAddress, label) => {
    ...
    const token = localStorage.getItem('token');
    if (token) {
      fetch('http://localhost:5555/api/wallets/label', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token
