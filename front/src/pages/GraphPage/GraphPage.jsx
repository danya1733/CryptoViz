...
function GraphPage() {
      ...
      try {
        ...
        if (token) {
          const response = await fetch(
            `http://localhost:5555/api/wallets/labels?cryptoType=${selectedCrypto}`,
            {
              headers: {
                Authorization: token,
              },
  ...
  const fetchAddedWallets = async () => {
    try {
      ...
      if (token) {
        const response = await fetch(
          `http://localhost:5555/api/wallets/?cryptoType=${selectedCrypto}`,
          {
            headers: {
              Authorization: token,
            },
  ...
  useEffect(() => {
    const fetchWalletImages = async () => {
      try {
        ...
        if (token) {
          const response = await fetch(
            `http://localhost:5555/api/wallets/images?cryptoType=${selectedCrypto}`,
            {
              headers: {
                Authorization: token,
              },
  ...
  const removeWallet = async (walletToRemove) => {
    try {
      ...
      if (token) {
        const response = await fetch(
          `http://localhost:5555/api/wallets/${walletToRemove}?cryptoType=${selectedCrypto}`,
          {
            method: 'DELETE',
            headers: {
              Authorization: token,
        ...
        if (response.ok) {
          ...
          await fetch(
            `http://localhost:5555/api/wallets/image?walletAddress=${walletToRemove}&cryptoType=${selectedCrypto}`,
            {
              method: 'DELETE',
              headers: {
                Authorization: token,
          ...
          await fetch(
            `http://localhost:5555/api/wallets/label?walletAddress=${walletToRemove}&cryptoType=${selectedCrypto}`,
            {
              method: 'DELETE',
              headers: {
                Authorization: token,
          ...
          if (selectedCrypto === 'btc') {
            ...
            for (const nodeId of [...nodesToRemove, ...transactionsToRemove]) {
              if (walletLabels[nodeId]) {
                await fetch(
                  `http://localhost:5555/api/wallets/label?walletAddress=${nodeId}&cryptoType=${selectedCrypto}`,
                  {
                    method: 'DELETE',
                    headers: {
                      Authorization: token,
              ...
              if (nodeImages[nodeId]) {
                await fetch(
                  `http://localhost:5555/api/wallets/image?walletAddress=${nodeId}&cryptoType=${selectedCrypto}`,
                  {
                    method: 'DELETE',
                    headers: {
                      Authorization: token,
